...
- 🔧 Preparazione sync (v0.9.1): `uuid` su `counters` e `rounds`, `updatedMs` sui `counters`, indici unici e migrazione Room 5→6 (solo ALTER TABLE e UPDATE, nessuna tabella ricreata). Tutte le scritture sui contatori passano da `save`/`create`, così il timestamp non resta indietro per distrazione — anche dai receiver delle notifiche. Nessun cambiamento visibile nell'app.
- 🐛 v0.9.2 — la ✏️ modifica ora rifasa la campanella quando sposti l'inizio del round: prima restava agganciata al vecchio orario, così "fallo partire da ieri alle 15" lasciava lo squillo dov'era. Le ricorrenti in "mantieni il ritmo" (FIXED) restano ferme di proposito.
- 🎛 v0.9.3 — ogni maschera si chiude senza fare nulla: intestazione unica con la ✕ in alto a destra su tutti gli sheet e i dialoghi. Prima la campanella già configurata mostrava solo Salva e Rimuovi, senza via d'uscita; gli "Annulla" sparsi sono stati tolti, la ✕ è l'unico modo e vale ovunque.
- Prossime: timer condivisi fra due dispositivi (Firebase), contatori giornalieri stile habit tracker, sezione segreta con PIN
